        ...
        <div class="max-w-3xl space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Add a new task to this project</p>
                </div>
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                <form method="POST" action="{{ route('projects.tasks.store', $project) }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @csrf

                    <div class="md:col-span-2">
                        <label class="block text-sm text-gray-700 mb-1">Title</label>
                        <input name="title" value="{{ old('title') }}" required
                               class="w-full rounded-md border-gray-300" placeholder="Task title">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="milestone_id" class="w-full rounded-md border-gray-300">
                            ...
                            @foreach($milestones as $ms)
                                <option value="{{ $ms->id }}" {{ (string)old('milestone_id') === (string)$ms->id ? 'selected' : '' }}>
                                    {{ $ms->title }}
                                </option>
                            @endforeach
                        </select>
